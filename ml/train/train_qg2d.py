"""Train the QG2D CNN surrogate with Generalized Teacher Forcing (GTF)."""
...
class QG2DDataset(Dataset):
    """Yields (x_start_noisy, x_target) pairs from a trajectory array.

    x_start:  (N*N,)          — state at time t, with Gaussian noise
    x_target: (K_steps, N*N)  — true states at t+1, ..., t+K_steps
    """

    def __init__(self, data: np.ndarray, K_steps: int, noise_std: float = NOISE_STD,
                 seed: int = 0):
        # data: (N_traj, T+1, N, N) → flatten spatial dims
        n, t1, h, w = data.shape
        self.data      = torch.from_numpy(data.reshape(n, t1, h * w)).float()
    ...
    def __getitem__(self, idx):
        ...
        x_target = self.data[i, t + 1 : t + 1 + self.K_steps]   # (K_steps, N*N)
        return x_start, x_target


def main():
    parser = argparse.ArgumentParser(description="Train QG2D CNN surrogate")
    ...
    train_data = npz["train"]   # (200, 501, 64, 64)
    val_data   = npz["val"]     # (30,  501, 64, 64)
    n_tr, t1, h, w = train_data.shape
    state_dim = h * w

    normalizer = Normalizer.from_data(
        torch.from_numpy(train_data.reshape(n_tr, t1, state_dim)))
    normalizer.mean = normalizer.mean.to(device)
    normalizer.std  = normalizer.std.to(device)

    model     = QG2DCNN(N=args.N, hidden_dim=args.hidden_dim,
    ...
    train_ds = QG2DDataset(train_data, K_steps=1, seed=args.seed)
    val_ds   = QG2DDataset(val_data,   K_steps=1, seed=args.seed + 1)
    ...
    # Save final epoch regardless of val loss (captures full K=10 training)
