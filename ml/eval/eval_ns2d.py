"""Evaluate the NS2D CNN surrogate: snapshots, VPT, energy spectrum, FTLE."""
...
from ml.utils.ns2d_physics import integrate, make_forcing

_CKPT_FINAL = os.path.join(os.path.dirname(__file__), '..', 'checkpoints',
                            'ns2d_cnn_final.pt')
_CKPT_BEST  = os.path.join(os.path.dirname(__file__), '..', 'checkpoints',
                            'ns2d_cnn_best.pt')
CKPT_PATH = _CKPT_FINAL if os.path.exists(_CKPT_FINAL) else _CKPT_BEST
DATA_PATH = os.path.join(os.path.dirname(__file__), '..', 'data',
                         'ns2d_trajectories.npz')
FIG_DIR   = os.path.join(os.path.dirname(__file__), '..', 'figures')
LAMBDA1       = 1.0           # placeholder; updated from truth FTLE during eval
...
def model_rollout_np(model, normalizer, omega0_flat_np, nsteps, device):
    """Free rollout from flat numpy IC. Returns (nsteps+1, N*N) numpy array."""
    x = torch.from_numpy(omega0_flat_np.astype(np.float32)).to(device)
    traj = model.rollout(x, nsteps, normalizer)
    return traj.cpu().numpy()


def _isotropic_spectrum(omega, N):
    """Isotropic kinetic energy spectrum E(k) from physical vorticity field ω (N,N)."""
...
def plot_snapshots(model, normalizer, test_data, dt, device, fig_dir, N):
    omega0_flat = test_data[0, 0].flatten()
    snap_steps  = [0, int(0.5 / dt), int(1.0 / dt), int(2.0 / dt)]
    nsteps      = max(snap_steps)

    pred_flat  = model_rollout_np(model, normalizer, omega0_flat, nsteps, device)
    ...
    fig.colorbar(im, ax=axes, label="ω", shrink=0.6)
    plt.tight_layout()
    path = os.path.join(fig_dir, "ns2d_snapshots.png")
...
def plot_vpt(model, normalizer, test_data, dt, device, fig_dir, N, n_ics=30):
    ...
    for i in range(n_ics):
        omega0_flat = test_data[i, 0].flatten()
        truth       = test_data[i].reshape(T + 1, state_dim).astype(np.float64)
        pred        = model_rollout_np(model, normalizer, omega0_flat, T, device)
    ...
    ax.set_title("NS2D CNN — Valid Prediction Time")
    ax.set_xlim(0, min(8 * LYAPUNOV_TIME, times[-1]))
    ax.set_ylim(0, 2.5)
    ax.legend(fontsize=8)
    plt.tight_layout()
    path = os.path.join(fig_dir, "ns2d_vpt.png")
    fig.savefig(path, dpi=150)
    plt.close(fig)
...
def plot_spectrum(model, normalizer, test_data, dt, device, fig_dir, N, npz_meta):
    omega0_flat = test_data[0, 0].flatten()
    nsteps      = 2000

    F_hat = make_forcing(N, k_f=int(npz_meta["k_f"]), F0=float(npz_meta["F0"]))
    pred_flat  = model_rollout_np(model, normalizer, omega0_flat, nsteps, device)
    truth_flat = integrate(test_data[0, 0].astype(np.float64), nsteps, dt, F_hat, N=N)
    ...
    k_f   = int(npz_meta["k_f"])
    k_ref = k_bins[2:].astype(float)

    # k^-3 reference (enstrophy cascade, k > k_f)
    k3_mask = k_ref > k_f
    k3_ref  = k_ref[k3_mask]
    k3_idx  = np.where(k3_mask)[0][0] + 2   # index into k_bins
    E_ref3  = k3_ref[0]**3 * Ek_truth_mean[k3_idx] * k3_ref**(-3)

    # k^-5/3 reference (inverse energy cascade, k < k_f)
    k53_mask = (k_ref > 1) & (k_ref < k_f)
    k53_ref  = k_ref[k53_mask]
    if len(k53_ref) > 0:
        k53_idx = np.where(k53_mask)[0][-1] + 2   # anchor at k just below k_f
        E_ref53 = k53_ref[-1]**(5/3) * Ek_truth_mean[k53_idx] * k53_ref**(-5/3)
    else:
        k53_ref = None
    ...
    ax.loglog(k3_ref, E_ref3, color="gray",   lw=1.0, ls=":",  label="$k^{-3}$")
    if k53_ref is not None:
        ax.loglog(k53_ref, E_ref53, color="green", lw=1.0, ls=":", label="$k^{-5/3}$")
    ax.axvline(k_f, color="k", lw=0.6, ls="--", alpha=0.5, label=f"$k_f={k_f}$")
    ax.set_xlabel("Wavenumber k")
    ax.set_ylabel("E(k)")
    ax.set_title("NS2D CNN — Isotropic Energy Spectrum")
    ax.legend(fontsize=9)
    plt.tight_layout()
    path = os.path.join(fig_dir, "ns2d_spectrum.png")
    fig.savefig(path, dpi=150)
    plt.close(fig)
    print(f"Saved {path}")
...
def plot_ftle(model, normalizer, test_data, dt, device, fig_dir, N, npz_meta,
              n_pairs=30, eps=0.01):
    ...
    for i in range(min(n_pairs, len(test_data))):
        omega0_flat = test_data[i, 0].flatten()
        dv          = rng.standard_normal(omega0_flat.shape)
        dv         /= np.linalg.norm(dv)
        omega0p_flat = omega0_flat + eps * dv

        # model FTLE
        ref_m  = model_rollout_np(model, normalizer, omega0_flat,  T, device)
        pert_m = model_rollout_np(model, normalizer, omega0p_flat, T, device)
        ...
        pert_t = integrate(omega0p_flat.reshape(N, N).astype(np.float64), T, dt,
                           F_hat, N=N).reshape(T + 1, state_dim)
    ...
    ax.set_title("NS2D CNN — FTLE Recovery")
    ax.set_xlim(0, min(10 * LYAPUNOV_TIME, times[-1]))
    ax.set_ylim(-2, 6)
    ax.legend(fontsize=9)
    plt.tight_layout()
    path = os.path.join(fig_dir, "ns2d_ftle.png")
    fig.savefig(path, dpi=150)
    plt.close(fig)
...
def main():
    ...
    dt        = float(npz["dt"])
    N         = int(npz["N"])
    npz_meta  = npz   # pass the full npz so helpers can read k_f, F0

    plot_snapshots(model, normalizer, test_data, dt, device, FIG_DIR, N)
    plot_vpt(      model, normalizer, test_data, dt, device, FIG_DIR, N)
    plot_spectrum( model, normalizer, test_data, dt, device, FIG_DIR, N, npz_meta)
    plot_ftle(     model, normalizer, test_data, dt, device, FIG_DIR, N, npz_meta)
